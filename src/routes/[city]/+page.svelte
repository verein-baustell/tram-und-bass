<script lang="ts">
  ...
  let randomIndex = Math.floor(Math.random() * ((releasedLines?.length ?? 0) - 1));
  ...
  onMount(() => {
    ...
    const keyHandlers = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight" && get(nextStation) && get(muxVideoObject)) {
        get(muxVideoObject).currentTime = get(currentTime) + get(timeUntilNextStation);
      }
      if (e.key === "ArrowLeft" && get(previousStation) && get(muxVideoObject)) {
        get(muxVideoObject).currentTime = hmsToSeconds(get(previousStation)!.startTime);
      }
      ...
      if (e.key === "Z" || e.key === "z") {
        ...
        if (state && state.line) changeToLineAtTime(state.line, state.time);
      }
      if (/^\d$/.test(e.key) && get(currentLine)?.timeStamps && get(muxVideoObject)) {
