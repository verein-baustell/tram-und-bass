...
currentCitySlug.subscribe((newCitySlug) => {
  if (newCitySlug) {
    ...
    const city = (content.cities as City[]).find((city) => city.slug === newCitySlug);
...
export const previousStation = derived(
  ...
  ([$currentTime, $currentLine]) => {
    const previousStation = $currentLine?.timeStamps
      ?.slice()
      .reverse()
      .find((timeStamp) => hmsToSeconds(timeStamp.startTime) < $currentTime);
...
export const lastPreviousStation = derived(
  ...
  ([$currentTime, $currentLine]) => {
    const lastPreviousStation = $currentLine?.timeStamps
      ?.slice()
      .reverse()
      .find((timeStamp) => hmsToSeconds(timeStamp.endTime) < $currentTime);
...
// Check if there's an existing state in local storage
const storedHistory =
  typeof localStorage != "undefined" ? localStorage.getItem(historyKey) : "[]";
